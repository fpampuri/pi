...
elemType elemAtIndex(listADT list, unsigned int index) {
    if(index >= list->size){
        exit(1);
        fprintf(stderr,"No se puede determinar");
    }
    List aux = list->first;
    while(index){
        index--;
...
void map(listADT list, elemType (*func)(elemType)){
    List aux = list->first;
    while(aux != NULL){
        aux->head = func(aux->head);
        aux = aux->tail;
    }
    return;
}